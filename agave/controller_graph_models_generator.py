...
def create_AAp_from_I_A(a, p, db="default"):
    for a_to in p.actors.using(db).exclude(id=a.id):
        if not AAp.objects.filter(actor_to=a,
                                  actor_from=a_to,
                                  instance=p):
            aap, created = AAp.objects.get_or_create(actor_from=a,
                                                     actor_to=a_to,
                                                     instance=p)
...
def create_CCp_from_I(c, p, db='default'):
    for c_to in p.concepts.using(db).exclude(id=c.id):
        if not CCp.objects.filter(concept_from=c_to,
                                  concept_to=c,
                                  instance=p):
            ccp, created = CCp.objects.get_or_create(concept_from=c,
                                                     concept_to=c_to,
                                                     instance=p)
...
#            ac = ActorConcept.objects.get(actor=a,concept=c)
